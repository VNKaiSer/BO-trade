          ...
          <tbody>
            <vs-tr :data="tr" :key="indextr" v-for="(tr, indextr) in data">
              ...
              <vs-td class="text-center whitespace-no-wrap hover:text-success">
                ...
                <vx-tooltip color="danger" text="Hủy xác minh">
...
export default {
  ...
  methods: {
      ...
      AuthenticationService.verifiedUser(obj).then((resq) => {
        if (resq.data.success) {
          return this.$vs.notify({
            text: val ? "Nạp thành công" : "Hủy nạp thành công",
            color: "success",
            iconPack: "feather",
            icon: "icon-check",
          });
    ...
    getOrderStatusColorText(verified) {
      if (verified == 0) return "Đang xử lý";
      if (verified == 1) return "Hoàn thành";
      //if(verified == 2) return "danger"
      return "Đang xử lý";
    },
