      ...
      <vs-table ref="table" pagination :max-items="itemsPerPage" search :data="products">
        ...
        <div slot="header" class="flex flex-wrap-reverse items-center justify-between flex-grow">
          ...
          <div class="flex flex-wrap-reverse items-center data-list-btn-container">
            ...
            <!-- ADD NEW -->
            <!-- <div class="flex items-center justify-center p-3 mb-4 mr-4 text-base text-lg font-medium border border-solid rounded-lg cursor-pointer btn-add-new text-primary border-primary" @click="addNewData">
          ...
          <template slot-scope="{data}">
            <tbody>
              <vs-tr :data="tr" :key="indextr" v-for="(tr, indextr) in data">
                ...
                <vs-td>
                    <p class="font-medium truncate verify-email">{{ tr.email }}<br>Số căn cước (CMT): {{ tr.so_cmnd ? tr.so_cmnd : 'Không rõ' }}</p>
                </vs-td>

                <vs-td>
                  <p class="font-medium truncate verify-name">{{ tr.first_name }}<br>Biệt danh: {{ tr.nick_name }}</p>
                </vs-td>
                ...
                <vs-td class="text-center whitespace-no-wrap hover:text-success">
                  <vx-tooltip  :title="tr.nick_name" color="success" text="Xác minh tài khoản">
