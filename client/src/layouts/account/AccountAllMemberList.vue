    ...
    <div id="loading-corners" class="vs-con-loading__container">
      ...
      >
        ...
        <template slot="thead">
          ...
          <vs-th sort-key="nickname">{{ $t("UJY") || "UJY" }}</vs-th>
          <vs-th sort-key="wallet">{{ $t("KUM") || "KUM" }}</vs-th>
          <vs-th sort-key="pending_commission">{{ $t("WER") || "WER" }}</vs-th>
          <vs-th sort-key="secury2fa">{{ $t("TWER") || "TWER" }}</vs-th>
          <vs-th sort-key="datecreate">{{ $t("TYTR") || "TYTR" }}</vs-th>
            ...
            <vs-tr :data="tr" :key="indextr" v-for="(tr, indextr) in data">
              ...
              <vs-td>
                <p class="font-medium truncate user-name">
                  ...
                  {{ $t("ERR") || "ERR" }} {{ tr.nick_name }}<br />
                  <vs-checkbox
                    v-model="tr.marketing"
                    @click="changeAccType(tr.id, tr.marketing)"
